...
class FinancialAssistanceRequest extends Mailable
{
    ...
    public function build()
    {
        ...
        $pdf = PDF::loadView('employee.reports.FinancialAssistanceAgreement', compact('record', 'results', 'perPart'));
    	
        $file = 'financial_assistance_agreement'.$record->employee->first_name.'_'.$record->employee->last_name.'_invoice.pdf';

        return $this->view('emails.admin.NewFinancialAssistanceRequest', compact('signature'))->subject('Financial Assistance Request')->attachData($pdf->output(), $file);
    }
}
